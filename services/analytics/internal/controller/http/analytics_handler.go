...
func NewAnalyticsHandler(analyticsRepo persistent.AnalyticsRepository, redisClient *redis.Client, logger *logger.Logger) *AnalyticsHandler {
...
func (h *AnalyticsHandler) GetCreatorStats(c *gin.Context) {
	...
	for _, post := range posts {
		if views, ok := post["views"].(int); ok {
			totalViews += views
		}
		postID, _ := post["id"].(string)
		if postID != "" {
			donationCount, err := h.analyticsRepo.GetPostDonations(postID)
			if err == nil {
				totalDonations += donationCount
			}
			likeCount, err := h.analyticsRepo.GetPostLikeCount(postID)
			if err == nil {
...
func (h *AnalyticsHandler) GetPostStats(c *gin.Context) {
	...
	creatorID, _ := post["creator_id"].(string)
	if creatorID != userID {
